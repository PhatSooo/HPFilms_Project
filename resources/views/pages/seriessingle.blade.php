            ...
            <div class="row ipad-width2">
                <div class="col-md-4 col-sm-12 col-xs-12">
                    <div class="movie-img sticky-sb">
                        <img src="{{ File::exists(public_path() . Storage::url($movie[0]->image)) ? Voyager::image($movie[0]->image) : 'https://image.tmdb.org/t/p/w500/' . $movie[0]->image }}"
                            alt="">
                        <div class="movie-btn">
                                ...
                                <div class="rate-star">
                                    ...
                                    @if (Session::has('userId'))
                                        <form method="POST" action="{{ route('rating', $movie[0]->slug) }}"
                                            ...
                                            <i class="ion-ios-star-outline" onmouseout=off(this.id)
                                                onmouseover=change(this.id) onclick=rate(this.id) id="1"></i>
                                            <input type="hidden" id="2_star" value="2" name="2">
                                            <i class="ion-ios-star-outline" onmouseout=off(this.id)
                                                onmouseover=change(this.id) onclick=rate(this.id) id="2"></i>
                                            <input type="hidden" id="3_star" value="3" name="3">
                                            <i class="ion-ios-star-outline" onmouseout=off(this.id)
                                                onmouseover=change(this.id) onclick=rate(this.id) id="3"></i>
                                            <input type="hidden" id="4_star" value="4" name="4">
                                            <i class="ion-ios-star-outline" onmouseout=off(this.id)
                                                onmouseover=change(this.id) onclick=rate(this.id) id="4"></i>
                                            <input type="hidden" id="5_star" value="5" name="5">
                                            <i class="ion-ios-star-outline" onmouseout=off(this.id)
                                                onmouseover=change(this.id) onclick=rate(this.id) id="5"></i>
                                            <input type="hidden" id="6_star" value="6" name="6">
                                            <i class="ion-ios-star-outline" onmouseout=off(this.id)
                                                onmouseover=change(this.id) onclick=rate(this.id) id="6"></i>
                                            <input type="hidden" id="7_star" value="7" name="7">
                                            <i class="ion-ios-star-outline" onmouseout=off(this.id)
                                                onmouseover=change(this.id) onclick=rate(this.id) id="7"></i>
                                            <input type="hidden" id="8_star" value="8" name="8">
                                            <i class="ion-ios-star-outline" onmouseout=off(this.id)
                                                onmouseover=change(this.id) onclick=rate(this.id) id="8"></i>
                                            <input type="hidden" id="9_star" value="9" name="9">
                                            <i class="ion-ios-star-outline" onmouseout=off(this.id)
                                                onmouseover=change(this.id) onclick=rate(this.id) id="9"></i>
                                            <input type="hidden" id="10_star" value="10" name="10">
                                            <i class="ion-ios-star-outline" onmouseout=off(this.id)
                                                onmouseover=change(this.id) onclick=rate(this.id) id="10"></i>
                                                    ...
                                                    <div class="cast-it">
                                                        <div class="cast-left series-it">
                                                            <img width="150" height="300"
                                                                src="{{ File::exists(public_path() . Storage::url($movie[0]->image)) ? Voyager::image($movie[0]->image) : 'https://image.tmdb.org/t/p/w500/' . $movie[0]->image }}"
                                                                alt="">
                                                            <div>
                                                                <a
                                                                    href="{{ URL::Route('series_single', $movie[0]->slug) }}">{{ $movie[0]->title }}</a>
                                                        ...
                                                        <div class="cast-it">
                                                            <div class="cast-left">
                                                                <img width="70" height="70"
                                                                    src="{{ File::exists(public_path() . Storage::url($actor->image)) ? Voyager::image($actor->image) : 'https://image.tmdb.org/t/p/w500/' . $actor->image }}"
                                                                    alt="">
                                ...
                                @foreach ($get_series as $seri)
                                    <div class="mvcast-item">
                                        <div class="cast-it">
                                            <div class="cast-left series-it">
                                                <img width="150" height="300"
                                                    src="{{ File::exists(public_path() . Storage::url($seri->image)) ? Voyager::image($seri->image) : 'https://image.tmdb.org/t/p/w500/' . $seri->image }}"
                                                    alt="">
                                                <div>
                                                    <a
                                                        href="{{ URL::Route('series_single', $seri->slug) }}">{{ $seri->title }}</a>
                                                    <p style="width:100%">{{ $seri->overview }}</p>
                                                    <p>{{ date_format(new DateTime($seri->date_release), 'M-d-Y') }}
                                                    </p>
                                                </div>
                                            </div>
                                        </div>
                                ...
                                @foreach ($list_rec as $item)
                                    <div class="movie-item-style-2">
                                        ...
                                        <div class="mv-item-infor">
                                            <h6><a
                                                    href="{{ $item->series != null ? URL::Route('series_single', $item->slug) : URL::Route('movie_single', $item->slug) }}">{{ $item->title }}
                                                    <span>({{ date_format(new DateTime($item->date_release), 'Y') }})</span></a>
                                            </h6>
                                            {{ $item->overview }}
                                            <p class="run-time"> Run Time:
                                                {{ date('h \h\ i \p', mktime(0, $item->runtime)) }} .
                                                <span>Release:
                                                    {{ date_format(new DateTime($item->date_release), 'd-M-Y') }}</span>
                                            </p>
